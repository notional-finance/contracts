...
describe("Liquidation", () => {
    ...
    describe("settle cash, local currency scenarios [1-3]", async () => {
        it("[1] should not do anything if the value is set to 0", async () => {
            await escrow.connect(wallet2).settleCashBalance(CURRENCY.DAI, CURRENCY.ETH, owner.address, wallet2.address, 0);
        });
        ...
        it("[1] should settle not cash between accounts when there is insufficient cash balance", async () => {
            ...
            await expect(escrow.connect(wallet2).settleCashBalance(CURRENCY.DAI, CURRENCY.ETH, owner.address, wallet2.address, WeiPerEther.mul(250)))
                .to.be.revertedWith(ErrorDecoder.encodeError(ErrorCodes.INCORRECT_CASH_BALANCE));
            await expect(escrow.settleCashBalance(CURRENCY.DAI, CURRENCY.ETH, wallet2.address, owner.address, WeiPerEther.mul(550)))
                .to.be.revertedWith(ErrorDecoder.encodeError(ErrorCodes.INCORRECT_CASH_BALANCE));
            await expect(escrow.settleCashBalance(CURRENCY.DAI, CURRENCY.ETH, owner.address, wallet2.address, WeiPerEther.mul(550)))
                .to.be.revertedWith(ErrorDecoder.encodeError(ErrorCodes.INCORRECT_CASH_BALANCE));
            await expect(escrow.settleCashBalance(CURRENCY.DAI, CURRENCY.ETH, wallet2.address, wallet.address, WeiPerEther.mul(500)))
                .to.be.revertedWith(ErrorDecoder.encodeError(ErrorCodes.INCORRECT_CASH_BALANCE));
        });
    ...
    describe("settle cash, trading scenarios [4-6]", async () => {
        it("[4] should partially settle cash when the account is undercollateralized", async () => {
            ...
            await escrow.settleCashBalance(CURRENCY.DAI, CURRENCY.ETH, wallet.address, owner.address, WeiPerEther.mul(100));
        ...
        it("[5] should revert when eth must be sold via uniswap and the price is out of line", async () => {
            ...
            await expect(escrow.settleCashBalance(CURRENCY.DAI, CURRENCY.ETH, wallet.address, owner.address, WeiPerEther.mul(100)))
    ...
    describe("liquidation scenarios", async () => {
        it("[1] should not liquidate an account that is properly collateralized", async () => {
            ...
            expect((await portfolios.freeCollateralView(wallet.address))[0]).to.be.above(0);
            await expect(escrow.liquidate(wallet.address, CURRENCY.DAI, CURRENCY.ETH))
        ...
        it("[2] should recollateralize an account using just liquidity tokens before it touches eth", async () => {
            ...
            await futureCash.addLiquidity(maturities[1], parseEther("90"), parseEther("90"), 1000);
        ...
        it("[3] should liquidate an account when it is under collateralized by eth", async () => {
            ...
            const closeOutCostETH = parseEther("105").mul(newRate).div(WeiPerEther).mul(parseEther("1.1")).div(WeiPerEther);
