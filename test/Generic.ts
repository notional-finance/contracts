...
describe("Generic Tests", () => {
    ...
    it("does not allow wallets to call protected functions on escrow", async () => {
        await expect(escrow.connect(wallet).unlockCollateral(2, futureCash.address, WeiPerEther))
            .to.be.revertedWith(ErrorDecoder.decodeError(ErrorCodes.UNAUTHORIZED_CALLER));
        await expect(escrow.connect(wallet).portfolioSettleCash(wallet.address, [WeiPerEther]))
            .to.be.revertedWith(ErrorDecoder.decodeError(ErrorCodes.UNAUTHORIZED_CALLER));

        await expect(escrow.connect(wallet).withdrawFromMarket(wallet.address, dai.address, 1, WeiPerEther, 0))
            .to.be.revertedWith(ErrorDecoder.decodeError(ErrorCodes.UNAUTHORIZED_CALLER));
        await expect(escrow.connect(wallet).depositIntoMarket(wallet.address, dai.address, 1, WeiPerEther, 0))
            .to.be.revertedWith(ErrorDecoder.decodeError(ErrorCodes.UNAUTHORIZED_CALLER));
    });

    it("does not allow wallets to call protected functions on future cash", async () => {
        await expect(futureCash.connect(wallet).setParameters(1, 0, 1e9, 1000, 4, 0))
            .to.be.revertedWith(ErrorDecoder.decodeError(ErrorCodes.UNAUTHORIZED_CALLER));
        await expect(futureCash.connect(wallet).settleLiquidityToken(wallet.address, WeiPerEther, maturities[0]))
            .to.be.revertedWith(ErrorDecoder.decodeError(ErrorCodes.UNAUTHORIZED_CALLER));

        await expect(futureCash.connect(wallet).tradeCashPayer(WeiPerEther, WeiPerEther, maturities[0]))
            .to.be.revertedWith(ErrorDecoder.decodeError(ErrorCodes.UNAUTHORIZED_CALLER));
        await expect(futureCash.connect(wallet).tradeCashReceiver(wallet.address, WeiPerEther, WeiPerEther, maturities[0]))
            .to.be.revertedWith(ErrorDecoder.decodeError(ErrorCodes.UNAUTHORIZED_CALLER));
        await expect(futureCash.connect(wallet).tradeLiquidityToken(WeiPerEther, WeiPerEther, maturities[0]))
            .to.be.revertedWith(ErrorDecoder.decodeError(ErrorCodes.UNAUTHORIZED_CALLER));
    });
    ...
    it("does not allow wallets to call protected functions on portfolios", async () => {
        ...
        await expect(portfolios.connect(wallet).raiseCollateralViaCashReceiver(wallet.address, 1, WeiPerEther))
            .to.be.revertedWith(ErrorDecoder.decodeError(ErrorCodes.UNAUTHORIZED_CALLER));
        await expect(portfolios.connect(wallet).raiseCollateralViaLiquidityToken(wallet.address, 1, WeiPerEther))
            .to.be.revertedWith(ErrorDecoder.decodeError(ErrorCodes.UNAUTHORIZED_CALLER));
