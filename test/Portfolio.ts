...
describe("Portfolio", () => {
    ...
    it("prevents trades being added past max trades", async () => {
      await portfolios.setMaxTrades(2);
    ...
    it("aggregates matching trades", async () => {
      await t.setupLiquidity();
      await t.borrowAndWithdraw(wallet, WeiPerEther.mul(100));
      await t.borrowAndWithdraw(wallet, WeiPerEther.mul(100));
    ...
    it("does not allow instrument groups with invalid currencies", async () => {
      await expect(
        portfolios.createInstrumentGroup(2, 40, 1e9, 3, futureCash.address, AddressZero)
      ).to.be.revertedWith(ErrorDecoder.encodeError(ErrorCodes.INVALID_CURRENCY));
    });

    it("allows instrument groups to be updated", async () =>{
      await portfolios.updateInstrumentGroup(1, 0, 1000, 1e8, CURRENCY.DAI, futureCash.address, owner.address);
      expect(await portfolios.getInstrumentGroup(1)).to.eql([0, 1000, 1e8, CURRENCY.DAI, futureCash.address, owner.address]);
    });
});